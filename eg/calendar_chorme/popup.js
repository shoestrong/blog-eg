document.addEventListener('DOMContentLoaded', function () {
  ...
  var N = (function() {
    function C() {
      var Z = Q.getJson();
      var c = Z.dateArray;
      M("cm").style.height = Z.lines * 42 + 2 + "px";
      M("cm").innerHTML = "";
      for (var a = 0; a < c.length; a++) {
        ...
        var X = R("DIV");
        if (c[a].isToday) {
          X.className = "cell active";
        }else{
          X.className = "cell";  
        }
        
        X.style.left = (a % 7) * 60 + "px";
        X.style.top = Math.floor(a / 7) * 55 + "px";
        var b = R("DIV");
        b.className = "so";
        b.style.color = ((a % 7) > 4 || c[a].isRest) ? "#FC625D" : "#444";
  ...
  var F = (function() {
    ...
    function G(b) {
      ...
      c += "农历 #{lunarMonthInChinese}月#{lunarDateInChinese}";
      c += "<br>#{ganzhiYear}年 #{ganzhiMonth}月 #{ganzhiDate}日";
      //if (a.solarFestival != "" || a.lunarFestival != "" || a.jieqi != "") {
       // c += "<br>#{lunarFestival} #{solarFestival} #{jieqi}"
     // }
      C.innerHTML = Y(c, a);
      var left=Z.offsetLeft + Z.offsetWidth - 50;
      var top=Z.offsetTop + Z.offsetHeight +5
      if(left>300)left=289;
      if(top>274)top=170;
      // console.log(top);
      C.style.top = top + "px";
